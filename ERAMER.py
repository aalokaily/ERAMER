...
print "Peptide ID\texpected length of epitope\tAverage of scores\tTrimming score of precursor 16\tTrimming score of precursor 15\tTrimming score of precursor 14\tTrimming score of precursor 13\tTrimming score of precursor 12\tTrimming score of precursor 11\tTrimming score of precursor 10\tTrimming score of precursor 9"
...
for line in fileinput.input(precursor_file):
    ...
    if tmp[0] == ">":
        ...
        precursor_seq = tmp[0].upper()
        
        #check if precusror contains non-amino-acidic content 
        f = ""
        ...
        if f == "non-amino-acidic content":
            print precursor_id + " contains non-amino-acidic content"
            
        elif not (9 <= prec_length <= 16):
            print "The length of precursor that typically processed by ERAP1 enzyme is 9-16 in length, " + precursor_id + " length is not within this length range"
        else:
            for epitope_length in epitope_lengths:
                ...
                if scores:
                    print precursor_seq + "\t" + str(epitope_length) + "\t" + str(sum(scores)/len(scores)) + "\t" + "\t".join(str(x) for x in scores) 
                else:
                    print
